...
- O gatilho que protegia fichas de outros jogadores bloqueava também o `ON DELETE SET NULL` feito internamente pelo banco ao excluir uma mesa. A nova regra libera apenas essa desvinculação automática quando a mesa já não existe; uma tentativa direta do mestre de desvincular ficha alheia continua proibida.
- A migração `202609250002_rpg_identity_invites.sql` foi aplicada no projeto `MiraculousRPGDB` com **Success**. Consulta de leitura posterior mostrou 3 mesas, 3 fichas, 10 mensagens e 3 convites com códigos distintos de seis dígitos. Nenhum registro existente foi apagado durante a aplicação. Os códigos antigos de 32 caracteres continuam válidos.
- A revisão encontrou que usar um convite conhecido zerava a contagem de tentativas inválidas. A migração complementar `202609250003_rpg_invite_rate_limit.sql` removeu essa possibilidade e foi aplicada no banco hospedado com **Success**. A definição da função hospedada não contém mais a limpeza da contagem; as mesmas 3 mesas, 3 fichas e 10 mensagens continuaram presentes.
- O cadastro distingue novo e-mail pendente de confirmação de um endereço já cadastrado que o Supabase devolve de forma disfarçada. Uma conta Discord conectada pode definir senha no perfil para usar o mesmo UUID por e-mail e, se houver nome de usuário, por nome. Não foi criada nem alterada nenhuma conta real para testar esse caminho.
- O carregamento inicial das campanhas deixou de esperar pelos bônus de fichas que só são necessários quando uma ficha é aberta. A tela de espera da campanha apresenta contexto, animação, aviso de demora, erro e tentativa novamente. Ainda não há medição real em rede móvel ou teste visual do desenho mobile final.
- `npm.cmd test`: 33 testes aprovados. `npm.cmd run build`: TypeScript/Vite aprovados, com aviso não bloqueante de pacote JavaScript acima de 500 kB. O teste isolado PostgreSQL/PGlite passou em 105 afirmações de permissões, exclusão com fichas vinculadas, convites e limite de tentativas. O teste isolado não substitui a exclusão de uma mesa descartável na conta real.
