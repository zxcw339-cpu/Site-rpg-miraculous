...
try {
  ...
  await as(master)
  const oldCampaignId = (await rows(`insert into public.rpg_campaigns(owner_id,name)
    values ('${master}','Convite anterior') returning id`))[0].id
  const oldCode = (await rows(`select public.rpg_create_campaign_invite('${oldCampaignId}') as code`))[0].code
  check(/^[a-f0-9]{32}$/.test(oldCode), true)
  await db.exec('reset role')
  const inviteMigration = await readFile(new URL('../supabase/migrations/202609250002_rpg_identity_invites.sql', import.meta.url), 'utf8')
  await db.exec(inviteMigration)
  await as(master)
  const newAlias = (await rows(`select public.rpg_create_campaign_invite('${oldCampaignId}') as code`))[0].code
  check(/^[0-9]{6}$/.test(newAlias), true)
  await db.exec('reset role')
  ...
  check(/^[0-9]{6}$/.test(code1), true)
  const code2 = (await rows(`select public.rpg_create_campaign_invite('${campaignId}') as code`))[0].code
  check(code1 === code2, true)
  await db.exec('reset role')
  const legacyCode = (await rows(`select code from rpg_private.campaign_invites where campaign_id='${campaignId}'`))[0].code
  check(/^[a-f0-9]{32}$/.test(legacyCode), true)
  await denied(`update rpg_private.campaign_invites set short_code='000000' where campaign_id='${campaignId}'`, /permanente/i)
  await as(master)
  ...
  check((await rows(`select public.rpg_join_campaign_by_code('invalido') as id`))[0].id, null)
  ...
  await db.exec('reset role')
  check((await rows(`select failures from rpg_private.invite_failures where user_id='${player}'`))[0].failures, 1)
  await as(player)
  check((await rows(`select public.rpg_join_campaign_by_code('${legacyCode}') as id`))[0].id, campaignId)
  check((await rows(`select public.rpg_join_campaign_by_code('${code2}') as id`))[0].id, campaignId)
  check(await rows(`delete from public.rpg_campaigns where id='${campaignId}' returning id`), [])
  ...
  for (let attempt = 0; attempt < 8; attempt++)
    check((await rows(`select public.rpg_join_campaign_by_code('bad') as id`))[0].id, null)
  check((await rows(`select public.rpg_join_campaign_by_code('${secondCode}') as id`))[0].id, null)
  await db.exec('reset role')
  check((await rows(`select failures from rpg_private.invite_failures where user_id='${outsider}'`))[0].failures, 8)
  await db.exec(`update rpg_private.invite_failures set window_start=now()-interval '16 minutes' where user_id='${outsider}'`)
  await as(outsider)
  check((await rows(`select public.rpg_join_campaign_by_code('${secondCode}') as id`))[0].id, secondCampaign)
  await db.exec(`reset role; delete from public.rpg_campaign_members where campaign_id='${secondCampaign}' and user_id='${outsider}'`)
  await as(outsider)
  ...
  check((await rows(`select public.rpg_join_campaign_by_code('${code1}') as id`))[0].id, campaignId)
  const linkedSheetId = (await rows(`insert into public.rpg_sheets(owner_id,campaign_id,name)
    values ('${player}','${campaignId}','Ficha que deve sobreviver') returning id`))[0].id

  await as(master)
  await db.exec(`insert into public.rpg_sheet_master_data(sheet_id,forms)
    values ('${linkedSheetId}','[]')`)
  await denied(`update public.rpg_sheets set campaign_id=null where id='${linkedSheetId}'`, /mestre|policy/i)
  ...
  check((await rows(`delete from public.rpg_campaigns where id='${campaignId}' returning id`))[0].id, campaignId)
  check(await rows(`select id from public.rpg_campaigns where id='${campaignId}'`), [])
  await db.exec('reset role')
  check((await rows(`select campaign_id from public.rpg_sheets where id='${linkedSheetId}'`))[0].campaign_id, null)
  check(await rows(`select sheet_id from public.rpg_sheet_master_data where sheet_id='${linkedSheetId}'`), [])
  await as(master)
  ...
  // A known valid code must not reset the failed-attempt budget.
  await as(player)
  for (let attempt = 0; attempt < 6; attempt++)
    check((await rows(`select public.rpg_join_campaign_by_code('bad') as id`))[0].id, null)
  check((await rows(`select public.rpg_join_campaign_by_code('${secondCode}') as id`))[0].id, secondCampaign)
  await db.exec('reset role')
  check((await rows(`select failures from rpg_private.invite_failures where user_id='${player}'`))[0].failures, 7)
  await as(player)
  check((await rows(`select public.rpg_join_campaign_by_code('bad') as id`))[0].id, null)
  check((await rows(`select public.rpg_join_campaign_by_code('${secondCode}') as id`))[0].id, null)
  await db.exec('reset role')
